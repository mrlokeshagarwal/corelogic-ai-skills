#!/usr/bin/env python3
"""unittest suite for compare_reports.py."""
...
class CompareReportsTests(unittest.TestCase):
    def test_detects_framework_and_version_changes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(data["frameworks_before"], ["net8.0"])
            self.assertEqual(data["frameworks_after"], ["net9.0"])
            self.assertEqual(
                data["packages_changed"],
                [
                    {
                        "project": "A.csproj",
